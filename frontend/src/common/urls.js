/** @format */

export const BASE_URL = 'http://localhost:4000/realEstate/api';
export const IMAGE_BASE_URL = 'http://localhost:4000/';

// export const BASE_URL = 'http://localhost/git/Massit_Admin2/public/api';
// export const IMAGE_BASE_URL = 'http://localhost/git/Massit_Admin2/public/';


export const GET_PROPERTIES_API = BASE_URL+'/getproperties/';
export const GET_PROPERTIES_WITHOUT_AUTH_API = BASE_URL+'/getpropertieswithoutAuth/';
export const GET_MY_PROPERTIES_API = BASE_URL+'/myproperties';

export const POST_SHOP_PAGE_API = BASE_URL+'/shopPage';
export const GET_PRODUCT_DETAILS_API = BASE_URL+'/productDetailWeb';
export const GET_PRODUCT_DETAILS_PAGE_API = BASE_URL+'/propertydetails/';
export const GET_ALL_COUNTRIES_API = BASE_URL+'/country';
export const POST_REGISTER_API = '/Userregister';
export const POST_LOGIN_API = BASE_URL + '/Userlogin';
export const GET_ALL_CATEGORY = BASE_URL+'/getcategories';
export const GET_ABOUT_US = BASE_URL+'/aboutus';
export const GET_PRIVACY_POLICY = BASE_URL+'/privacypolicy';
export const GET_TERMS_CONDITIONS = BASE_URL+'/termsandconditions';
export const GET_BLOGS = BASE_URL+'/blog';
export const GET_CONTACTUS_DETAILS = BASE_URL+'/contactus';
export const POST_REVIEW_API = BASE_URL+ '/userreview';
export const POST_SUBMIT_REVIEW_API = BASE_URL+'/reviewsubmit/';
export const GET_REVIEW_DETAILS = BASE_URL+'/reviews/';
export const POST_ADD_FAVORITE_API = BASE_URL+'/addToFavorite';
export const POST_ADD_PROPERTY_API = BASE_URL+'/createproperty';
export const GET_FAVORITE_PROPERTY_API = BASE_URL+'/myFavorites';
export const DELETE_FAVORITE_PROPERTY_API = BASE_URL+'/deleteFavorite';
export const GET_ALL_CITIES = BASE_URL+'/city';
export const GET_ALL_AMENITIES = BASE_URL+'/amenities';

